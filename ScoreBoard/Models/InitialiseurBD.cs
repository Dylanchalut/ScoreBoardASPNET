namespace ScoreBoard.Models
{
    public class InitialiseurBD
    {
        ...
        private static Dictionary<string, Joueur> _JoueurDict;
        private static Dictionary<string, Joueur> JoueurDict
        {
            get
            {
                _JoueurDict = new Dictionary<string, Joueur>();
                foreach (Joueur joueur in _listeJoueurs)
                {
                    _JoueurDict.Add(joueur.Nom, joueur);
                }
                return _JoueurDict;
            }
        }
        ...
        public static void Seed(IApplicationBuilder applicationBuilder)
        {
            ...
            CatalogueJoueursBdContext catalogueGateauxDbContext =
            applicationBuilder.ApplicationServices.CreateScope()
            .ServiceProvider.GetRequiredService<CatalogueGateauxDbContext>();

            if (!catalogueGateauxDbContext.Gateaux.Any())
            {
                catalogueGateauxDbContext.Gateaux.AddRange(NomGateauDict.Values);
            }

            catalogueGateauxDbContext.SaveChanges();

            if (!catalogueGateauxDbContext.Ingredients.Any())
            {
                catalogueGateauxDbContext.Ingredients.AddRange(_ingredients);
            }
